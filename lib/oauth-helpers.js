...
// Plain-HTTP localhost dev servers. Allowed everywhere except production.
...
export function isAllowedOrigin(origin) {
  ...
  if (process.env.VERCEL_ENV !== 'production' && DEV_ALLOWED_ORIGINS.has(origin)) return true;
...
export function setCors(req, res) {
  ...
  // Vary: Origin unconditionally so caches never serve one origin's CORS
  // headers to another, whether or not this origin was allowed.
...
export function parseCookies(cookieHeader) {
  ...
  cookieHeader.split(';').forEach(part => {
    ...
    catch { out[key] = val; }  // malformed %-escapes: keep the raw value
  });
  return out;
}
